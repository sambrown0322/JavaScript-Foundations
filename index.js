...
let monthlyRate = p * (n2 / denominator);
...
function mortgageCalculator(p, i, n, c) {
  ...
  let monthlyRate = p * (n2 / denominator);
  if (c > 740) {
    monthlyRate = monthlyRate * 0.995;
  } else if (c < 660) monthlyRate = monthlyRate * 1.005;
  console.log(`${name}, your monthly rate is ${monthlyRate}.`);
}
...
function variableInterestRate(p, i, n) {
  ...
  for (let x = 0; x < 9; x++) {
    let y = i / 12;
    let z = n * 12;
    let n1 = Math.pow(1 + y, z);
    let n2 = n1 * y;
    let denominator = n1 - 1;
    let monthlyRate = p * (n2 / denominator);
    let rounded = monthlyRate.toFixed(2);
    let istring = i.toFixed(3);
    console.log(
      `${name}, with an interest rate of ${istring}, your monthly rate is ${rounded}.`
    );
    i = i + 0.005;
  }
}
